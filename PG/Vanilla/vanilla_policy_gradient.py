#############################################################
# This file implement vanilla policy gradient algorithm.
# The algorithm demonstration in vanilla_policy_gradient.md.
# Vanilla policy gradient is only for 1-dimentional state.
...
class TrainAgent():
	...
	def _target_policy(self, states, actions, advs):
		...
		logprob = self.agent.policy.log_likelihood(states_t, actions)
		return -(logprob.squeeze(0) * advs_t).sum()
	...
	def _target_value(self, states, rewards_go):
		...
		return ((values_t - rewards_go_t) ** 2).mean()
